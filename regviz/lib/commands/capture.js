...
import { addBaseline, addTarget, getDataPath, ensureDirectory } from '../config.js';
...
export async function executeCaptureCommand(options) {
  try {
    ...
    // Determine the role of this screenshot set (for backward compatibility)
    const role = ['main', 'release'].includes(source) ? 'baseline' : 'target';
    ...
    addScreenshotSet(name, {
      ...snapshotData,
      role,  // Store the original role for compatibility
      directory: outputDir,
      source,
